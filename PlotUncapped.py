...
# EXPANDED COLOR PALETTE FOR MORE PEOPLE
PERSON_COLORS = [
...
def get_person_keypoints(frame_data, person_index):
    ...
    Returns None, None, None if the person index doesn't exist in this frame.
    """
    detections = frame_data.get("detections")
    
    # Check if detections exist and if the requested index is within bounds
    if (detections and 
...
class MultiPose3DPlayer:
    def __init__(
        ...
        # --- DYNAMICALLY CALCULATE MAX PEOPLE ---
        # Scan all frames to find the highest number of detections in any single frame
        max_detected = 0
        ...
        # Ensure at least 1 slot if data is empty, otherwise use the max found
        self.max_people = max_detected if max_detected > 0 else 1
        print(f"Loaded {self.num_frames} frames. Max people detected in a single frame: {self.max_people}")
        ...
        # Collect points from ALL potential people in the first frame to set limits
        ...
        # If first frame is empty, try to set default limits or scan forward (simple fallback here)
        if not all_x:
            print("Warning: No keypoints found in Frame 0. Defaulting limits.")
        ...
        # Create artists for the maximum possible number of people
        for p_id in range(self.max_people):
            # Cycle through colors if we have more people than colors defined
            color = PERSON_COLORS[p_id % len(PERSON_COLORS)]
            
            # Initialize with Frame 0 data if available, else empty
            x, y, z = get_person_keypoints(self.frames[0], p_id)
            ...
            if x is not None:
                ...
                # Create empty artists placeholders
    ...
    def _set_limits(self, x, y, z, margin_factor=1.05, initial_setup=False, default=False):
        if initial_setup:
            ...
            if np.isnan(x_min) or np.isnan(x_max):
                 print("Warning: Keypoints in first frame are NaN. Setting default limits.")
                 self.ax.set_xlim(-2000, 2000)
                 self.ax.set_ylim(-2000, 2000)
                 self.ax.set_zlim(0, 4000)
                 return
    ...
    def _add_widgets(self):
        ...
        # Row 2: Custom Keypoint Input & Reference Height Input
    ...
    def _draw_frame(self, i):
        ...
        # Loop through the maximum number of people slots we created
        for p_id in range(self.max_people):
    ...
    def _on_height_submit(self, text):
        try:
            ...
            if x_raw is None or len(y_raw) <= max(REF_HEAD_IDX, REF_FOOT_IDX):
                 print("[SCALING ERROR] Target person not detected or reference keypoints missing.")
