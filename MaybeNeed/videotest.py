...
W, H = 640, 480
TARGET_FPS = 29.97  # The capped framerate
FRAME_DURATION = 1.0 / TARGET_FPS  # Seconds per frame budget
CONFIDENCE_THRESHOLD = 0.1

# --- VIDEO WRITER CONFIG ---
video_path = os.path.join(OUTPUT_DIR, "skeleton_tracking_output.mp4")
fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
# We use the TARGET_FPS here so the header of the MP4 file is correct
out = cv2.VideoWriter(video_path, fourcc, TARGET_FPS, (W, H))

# Start the RealSense pipeline
pipeline = rs.pipeline()
config = rs.config()
# RealSense hardware rounds to 30, which aligns closely with 29.97
...
try:
    frame_index = 0
    print(f"Recording started at {TARGET_FPS} FPS... Press 'q' to stop.")
    ...
    while True:
        ...
        # YOLOv11 Pose Estimation (Will be very fast on 5080)
        ...
        # Add metadata to visual frame
        cv2.putText(color_image, f"FPS CAP: {TARGET_FPS} | Frame: {frame_index}", 
        ...
        # --- FPS LIMITER ---
        # Calculate how long processing took and sleep for the remainder
